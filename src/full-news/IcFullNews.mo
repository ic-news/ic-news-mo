...
actor class News() = this{
    ...
    stable var _news_array : [FullNewsTypes.FullNews] = [];
    var _news_buffer = Buffer.Buffer<FullNewsTypes.FullNews>(0);

    stable var _category_array : [NewsTypes.Category] = [];

    stable var _tag_array : [NewsTypes.Tag] = [];

    stable var _archives : [FullNewsTypes.FullArchiveData] = [];
    ...
    public query func get_task_status(): async  Result.Result<(Bool, Text), NewsTypes.Error> {
        return #ok((_task_status, _last_error_message));
    };

    public query func get_archives(): async  Result.Result<[FullNewsTypes.FullArchiveData], NewsTypes.Error> {
        return #ok(_archives);
    };

    public query func get_providers(): async  Result.Result<[(Principal,Text)], NewsTypes.Error> {
        let providers = StableTrieMap.entries(_provider_store);
        let provider_list = Iter.toArray(providers);
        return #ok(provider_list);
    };

    public query func get_categories(): async  Result.Result<[NewsTypes.Category], NewsTypes.Error> {
        return #ok(_category_array);
    };

    public query func get_tags(): async  Result.Result<[NewsTypes.Tag], NewsTypes.Error> {
        return #ok(_tag_array);
    };

    public shared(msg) func add_provider(pid : Principal, alias : Text): async  Result.Result<Bool, NewsTypes.Error> {
    ...
    public shared(msg) func add_categories(categories: NewsTypes.AddCategoryArgs): async  Result.Result<Bool, NewsTypes.Error> {
        ...
        switch(provider){
            ...
            case (?provider) {
                for(category in categories.args.vals()){
                    switch(Array.find<NewsTypes.Category>(_category_array, func(a: NewsTypes.Category): Bool {
    ...
    public shared(msg) func add_tags(tags: NewsTypes.AddTagArgs): async  Result.Result<Bool, NewsTypes.Error> {
        let provider = StableTrieMap.get(_provider_store, Principal.equal, Principal.hash, msg.caller);
        switch(provider){
            case null {
                return #err(#InternalError("Provider not found"));
            ...
            case (?provider) {
                for(tag in tags.args.vals()){
                    switch(Array.find<NewsTypes.Tag>(_tag_array, func(a: NewsTypes.Tag): Bool {
                        a.name == tag.name
                    })){
                        case null {
    ...
    public shared(msg) func add_news(news: FullNewsTypes.AddFullNewsArgs): async  Result.Result<Bool, NewsTypes.Error> {
        let provider = StableTrieMap.get(_provider_store, Principal.equal, Principal.hash, msg.caller);
        switch(provider){
            case null {
                return #err(#InternalError("Provider not found"));
            ...
            case (?provider) {
                let buffer = Buffer.Buffer<FullNewsTypes.FullNews>(news.args.size());
                for(news_arg in news.args.vals()){
                    let news_item : FullNewsTypes.FullNews = {news_arg with
                        index = _index;
    ...
    public composite query func query_news(req: NewsTypes.NewsRequest): async  FullNewsTypes.FullNewsResponse {
        ...
        var news_in_canister : [FullNewsTypes.FullNews] = [];
        let buffer = Buffer.Buffer<FullNewsTypes.FullNews>(req.length);
        ...
        let archive_news = Buffer.Buffer<FullNewsTypes.ArchivedFullNews>(_archives.size());
    ...
    public query func total_news() : async  Result.Result<Nat, NewsTypes.Error> {
    ...
    public query func query_latest_news(size: Nat): async  Result.Result<[FullNewsTypes.FullNews], NewsTypes.Error> {
        return #ok(_query_latest_news(size));
    };

    private func _query_latest_news(size: Nat):[FullNewsTypes.FullNews] {
        let news_list = Buffer.toArray(_news_buffer);
        var sortedArray = Array.sort(news_list, func(a: FullNewsTypes.FullNews, b: FullNewsTypes.FullNews) : Order.Order {
            Nat.compare(a.index, b.index)
        });
        sortedArray := Array.reverse(sortedArray);
        let buffer = Buffer.Buffer<FullNewsTypes.FullNews>(size);
    ...
    public query func get_news_by_hash(hash: Text): async  Result.Result<FullNewsTypes.FullNews, NewsTypes.Error> {
        let news_list = Buffer.toArray(_news_buffer);
        for(news_item in news_list.vals()){
    ...
    public composite query func get_news_by_index(index: Nat): async  Result.Result<FullNewsTypes.FullNews, NewsTypes.Error> {
        let news_list = Buffer.toArray(_news_buffer);
        for(news_item in news_list.vals()){
    ...
    public query func get_news_by_time(begin_time : Nat, size: Nat): async  Result.Result<[FullNewsTypes.FullNews], NewsTypes.Error> {
        let buffer = Buffer.Buffer<FullNewsTypes.FullNews>(size);
        let news_list = Buffer.toArray(_news_buffer);
        let sortedArray = Array.sort(news_list, func(a: FullNewsTypes.FullNews, b: FullNewsTypes.FullNews) : Order.Order {
    ...
    type AppMessage = {
        topic : Text;
        args : [Text];
        result : FullNewsTypes.WebSocketValue;
        timestamp : Nat64;
    };
    ...
    private func _save_to_archive() : async () {
        ...
        try{
            ...
            if(total > 12000){
                ...
                _news_buffer := Buffer.fromArray<FullNewsTypes.FullNews>(_remain_array);

                //get last archive from archives
                var last_archive : FullNewsTypes.FullArchiveInterface = actor("aaaaa-aa");
                var archive_data : FullNewsTypes.FullArchiveData = { 
    ...
    private func _deploy_archive_canister() : async FullNewsTypes.FullArchiveInterface {
    ...
    system func postupgrade() {
        _news_buffer := Buffer.fromArray<FullNewsTypes.FullNews>(_news_array);
    };
