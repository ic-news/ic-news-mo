...
import Types "../types/FullNewsTypes";
...
shared (initMsg) actor class Archive() = this {
    ...
    public shared ({ caller }) func append_news(newses : [Types.FullNews]) : async Result.Result<Bool, NewsTypes.Error> {
        if (not Principal.isController(caller)) {
            return #err(#InternalError("Unauthorized Access: Can not access this archive canister"));
        };
        if(newses.size() == 0) return #ok(true);
        let buffer = Buffer.Buffer<Types.FullNews>(newses.size());
    ...
    public query func get_news(block_index : Nat) : async Result.Result<Types.FullNews, NewsTypes.Error> {
    ...
    private func _get_news(block_index : Nat) : ?Types.FullNews {
    ...
    public query func query_news(req : Types.NewsRequest) : async Types.FullNewsRange {
        return _query_news(req.start, req.length);
    };

    private func _query_news(start : Nat, length : Nat) : Types.FullNewsRange {
    ...
    public query func remaining_capacity() : async Result.Result<Nat, NewsTypes.Error> {
        return #ok(MAX_MEMORY - Nat64.toNat(total_memory_used));
    };

    public query func get_cycle_balance() : async Result.Result<Nat, NewsTypes.Error> {
        return #ok(Cycles.balance());
    };

    private func _to_blob(tx : Types.FullNews) : Blob {
        to_candid (tx);
    };

    private func _from_blob(tx : Blob) : Types.FullNews {
        switch (from_candid (tx) : ?Types.FullNews) {
            case (?tx) tx;
            case (_) Debug.trap("Could not decode tx blob");
        };
    };

    private func _store_data(tx : Types.FullNews) : MemoryBlock {
    ...
    private func _get_data({ offset; size } : MemoryBlock) : Types.FullNews {
